...
function upsertUser(telegramId, chatId, extra) {
  ...
  if (!existing) data.created_at = new Date();
  return firestoreUpdate('users/' + telegramId, data);
}

function getUserRoutes(telegramId) {
  return firestoreList('users/' + telegramId + '/routes');
}
...
function getUserFilters(telegramId) {
  return firestoreGet('users/' + telegramId + '/settings/filters');
}
...
function listActiveUsers() {
  return firestoreList('users').filter(function (u) { return u.status === 'active'; });
}
