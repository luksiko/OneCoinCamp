...
function getOffers(filter, initData) {
  ...
  const offers = values.map(function (row) {
    return {
      ...
      pickupDate: row[9] instanceof Date ? Utilities.formatDate(row[9], (spreadsheet && spreadsheet.getSpreadsheetTimeZone) ? spreadsheet.getSpreadsheetTimeZone() : Session.getScriptTimeZone(), 'yyyy-MM-dd') : String(row[9] || ''),
      returnDate: row[10] instanceof Date ? Utilities.formatDate(row[10], (spreadsheet && spreadsheet.getSpreadsheetTimeZone) ? spreadsheet.getSpreadsheetTimeZone() : Session.getScriptTimeZone(), 'yyyy-MM-dd') : String(row[10] || ''),
      price: row[11],
