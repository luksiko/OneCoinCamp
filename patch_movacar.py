...
start_marker = "class MovacarProvider:"
end_marker = "class IndieCampersProvider:"

start_idx = content.find(start_marker)
end_idx = content.find(end_marker)

new_class = """class MovacarProvider:
    base_url = "https://crowd-api-production-615013621295.europe-west1.run.app"

    def __init__(self, http) -> None:
        self.http = http

    def fetch_destinations(self, origin_reference: str) -> list[dict]:
        query = __import__("urllib.parse").parse.urlencode({"locale": "de", "origin_reference": origin_reference})
        payload = self.http.get(
            ...
            headers={
                ...
                "X-Request-Id": __import__("uuid").uuid4().hex[:12],
            },
        )
    ...
    def fetch_offers(
        self,
        route,
        allowed_destination_countries = None,
    ) -> list:
        ...
        query = __import__("urllib.parse").parse.urlencode(query_params)
        try:
            payload = self.http.get(
        ...
        for item in payload.get("data", []):
            ...
            # Simple country check skipped here, handled by gas/Providers.js mainly or allowed_destination_countries
